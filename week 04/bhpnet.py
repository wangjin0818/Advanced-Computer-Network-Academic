...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hle:t:p:cu:", ["help", "listen", "execute", "target", "port", "command", "upload"])  # @UnusedVariable
    ...
    # are we going to listen or just send data fron stdin?
...
def client_sender(buffer):  # @ReservedAssignment
    client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # @UnusedVariable
    try:
        ...
        while True:
            ...
            # wait for more input
            buffer = raw_input("")  # @ReservedAssignment
            buffer += "\n"
